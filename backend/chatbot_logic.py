...
async def process_chat_message_async(message: str, state: dict) -> dict:
    """대화 상태에 따라 비동기로 템플릿 생성 및 수정을 관리"""
        ...
        if message in STYLE_OPTIONS:
            ...
            try:
                ...
                if not original_request:
                    state['step'] = 'initial'
                    return {'message': '오류: 최초 요청을 찾을 수 없습니다. 처음부터 다시 시작해주세요.', 'state': state}

                print(f"⏳ '{state['selected_style']}' 스타일로 '{original_request}' 내용 생성 중...")
                ...
                response = TemplateResponse(style=state["selected_style"], template_data=template_obj)
                state["final_template_response"] = response.model_dump()
                state["step"] = "awaiting_feedback_new"
                
                response_payload = {
                    "message": "✅ 템플릿 생성이 완료되었습니다. 수정하고 싶은 부분이 있다면 말씀해주세요.",
                    "template_response": state["final_template_response"],
                    "options": ["완료"],
                }
                ...
                state["step"] = "select_style_new"
                response_payload = {
                    "message": f"⚠️ 생성 중 오류가 발생했습니다: {e}\n내용을 조금 더 구체적으로 작성하여 다시 요청해주세요.",
                    "options": STYLE_OPTIONS
                }
        else:
            response_payload = {"message": "잘못된 스타일입니다. 제시된 옵션 중에서 선택해주세요.", "options": STYLE_OPTIONS}

    # --- 생성된 템플릿에 대한 피드백 대기 및 처리 ---
    elif step == "awaiting_feedback_new":
        if message == "완료":
            response_payload = {"message": "이용해주셔서 감사합니다! 새로운 템플릿을 만들려면 아무 내용이나 입력해주세요."}
            state.clear()
            state["step"] = "initial"
        else:
            try:
                print(f"✍️ 피드백 반영 중: {message}...")
                ...
                response_payload = {
                    "message": "수정이 완료되었습니다. 더 수정할 부분이 있나요?",
                    "template_response": state["final_template_response"],
                    "options": ["완료"],
                }
            except Exception as e:
                print(f"Error during refinement: {e}")
                response_payload = {"message": f"⚠️ 수정 중 오류가 발생했습니다: {e}\n다른 방식으로 다시 요청해주세요."}
    ...
    # --- [기존 코드 흐름] 유사 템플릿 선택 후 완료 단계 ---
        ...
        message_text = f"""✅ 최종 템플릿이 완성되었습니다!\n\n```\n{final_template_text}\n```\n\n"""
        if variables_info:
            message_text += "추출된 변수 정보:\n"
            for var in variables_info:
                message_text += f"- {var['name']} (원본: {var['original_value']}): {var['description']}\n"
        
        message_text += "\n이 템플릿을 사용하시겠습니까? 아니면 추가 수정이 필요하신가요?"
        
        state['step'] = 'final_review_new_style'
        return {'message': message_text, 'state': state, 'options': ['사용', '수정']}

    # --- [기존 코드 흐름] 최종 검토 (사용/수정) ---
    elif state['step'] == 'final_review_new_style':
        if message == '사용':
            state['step'] = 'initial'
            return {'message': '템플릿이 최종 확정되었습니다. 새로운 템플릿을 만들려면 아무 내용이나 입력해주세요.', 'state': state, 'final_template': state['base_template']}
        elif message == '수정':
            state['step'] = 'awaiting_user_feedback_old_style'
            return {'message': f'어떤 부분을 수정하시겠어요? 구체적으로 알려주세요.\n\n현재 템플릿:\n```{state["base_template"]}```', 'state': state}
        else:
            return {'message': "잘못된 입력입니다. '사용' 또는 '수정'으로만 답해주세요.", 'state': state, 'options': ['사용', '수정']}
    ...
    # --- [기존 코드 흐름] 사용자 피드백 대기 및 처리 ---
        ...
        if validation_result['status'] == 'accepted':
            ...
            return {'message': f'수정된 템플릿에 문제가 발견되었습니다.\n\n문제점: {validation_result["reason"]}\n\n개선 제안: {validation_result.get("suggestion", "없음")}\n\nAI가 다시 수정하거나 직접 수정하시겠습니까?', 'state': state, 'options': ['AI 수정', '직접 수정']}

    # --- [기존 코드 흐름] 피드백 후 수정 (AI/직접) ---
        ...
        if message == 'AI 수정':
            ...
            return {'message': f'현재 템플릿 초안입니다. 직접 수정할 내용을 입력해주세요.\n\n```{state["template_draft"]}```', 'state': state}
        else:
            return {'message': "잘못된 입력입니다. 'AI 수정' 또는 '직접 수정'으로만 답해주세요.", 'state': state, 'options': ['AI 수정', '직접 수정']}
    ...
    # --- [기존 코드 흐름] AI 자동 수정 ---
        ...
        if state.get('correction_attempts', 0) < MAX_CORRECTION_ATTEMPTS:
            ...
            if validation_result["status"] == "accepted":
                ...
                state['template_draft'] = corrected_template
                return {'message': f'AI 자동 수정 후에도 문제가 발견되었습니다. (시도: {state["correction_attempts"]}/{MAX_CORRECTION_ATTEMPTS})\n\n문제점: {validation_result["reason"]}\n\nAI가 다시 한번 수정합니다.', 'state': state}
        else:
            # 여기가 문제의 원인이었던 부분입니다. else 블록의 들여쓰기를 수정했습니다.
            state['step'] = 'manual_correction_old_style'
            return {'message': f'AI 자동 수정({MAX_CORRECTION_ATTEMPTS}회)에 실패했습니다. 직접 수정하시겠습니까?', 'state': state, 'options': ['예', '아니오']}
    ...
    # --- [기존 코드 흐름] 수동 수정 ---
        ...
        if message == '예':
            ...
            state['step'] = 'initial'
            return {'message': '템플릿 생성을 취소하고 초기 상태로 돌아갑니다.', 'state': state}
        else:
            return {'message': "잘못된 입력입니다. '예' 또는 '아니오'로만 답해주세요.", 'state': state, 'options': ['예', '아니오']}

    # --- [기존 코드 흐름] 사용자 직접 수정 입력 대기 ---
        ...
        if validation_result['status'] == 'accepted':
            ...
            return {'message': f'수정하신 템플릿에 문제가 발견되었습니다.\n\n문제점: {validation_result["reason"]}\n\n개선 제안: {validation_result.get("suggestion", "없음")}\n\n다시 수정하시겠습니까?', 'state': state, 'options': ['예', '아니오']}

    # --- [기존 코드 흐름] 수동 수정 피드백 ---
    elif state['step'] == 'manual_correction_feedback_old_style':
        if message == '예':
            state['step'] = 'awaiting_user_correction_old_style'
            return {'message': f'현재 템플릿 초안입니다. 다시 수정할 내용을 입력해주세요.\n\n```{state["template_draft"]}```', 'state': state}
        elif message == '아니오':
            state['step'] = 'initial'
            return {'message': '템플릿 생성을 취소하고 초기 상태로 돌아갑니다.', 'state': state}
        else:
